...
本文档从目录划分、架构、提示词系统、模块实现与关联、新人开发指南、UML 图等方面对项目进行完整分析。
...
├── market_analyzer.py       # 大盘复盘（指数/板块/北向/新闻 + LLM 报告）
...
│   ├── akshare_fetcher.py   # AkShare（日线 + 实时行情 + 筹码分布）
...
| **大盘复盘** | 指数/板块/北向/新闻 + LLM 生成复盘报告 | `market_analyzer.py`（MarketAnalyzer） |
...
- **角色**：`SYSTEM_PROMPT` 定义「专注趋势交易的 A 股投资分析师」，输出为**决策仪表盘**。
- **交易理念（硬约束）**：
  - **严进**：乖离率 (现价-MA5)/MA5 > 5% 不买，直接观望。
  - **趋势**：多头排列 MA5 > MA10 > MA20。
  - **效率**：筹码 90% 集中度 < 15%、获利比例等。
  - **买点**：缩量回踩 MA5/MA10 支撑为佳。
  - **风险**：减持、业绩变脸、监管、解禁等必须体现。
...
- **角色**：`_build_review_prompt` 中定义为「专业 A 股市场分析师」，根据当日市场数据生成**纯 Markdown 复盘报告**。
- **输入**：`MarketOverview`（日期、主要指数、涨跌家数、涨停跌停、成交额、北向、领涨领跌板块）+ 搜索得到的市场新闻列表。
- **格式约束**：纯 Markdown、禁止 JSON/代码块、emoji 仅标题少量使用。
- **输出模板**：  
  「📊 日期 大盘复盘」→ 一、市场总结 → 二、指数点评 → 三、资金动向 → 四、热点解读 → 五、后市展望 → 六、风险提示。
...
2. **本地跑通**：  
   ...
   - 指定股票：`python main.py --stocks 600519,000001`  
...
classDiagram
    ...
    DatabaseManager --> StockDaily : 持久化
```
...
flowchart LR
    ...
    subgraph 提示词
        SP[SYSTEM_PROMPT<br/>交易理念+JSON结构]
